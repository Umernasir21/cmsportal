...
export default function LoginPage({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0F2044 0%,#1E3A5F 60%,#0D47A1 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, system-ui, sans-serif', padding: 16 }}>
      <div className="login-grid fade-in" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', background: '#fff', borderRadius: 20, overflow: 'hidden', width: '100%', maxWidth: 860, boxShadow: '0 32px 80px rgba(0,0,0,0.4)' }}>

        {/* Left panel — hidden on mobile */}
        <div className="login-left" style={{ background: 'linear-gradient(160deg,#0F2044,#1E3A5F)', padding: '48px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 36 }}>
              ...
              <div>
                <div style={{ color: '#fff', fontWeight: 800, fontSize: 17 }}>CMS Portal</div>
                <div style={{ color: '#546E7A', fontSize: 11 }}>Complaint Management System</div>
              </div>
            </div>
        ...
        <div className="login-form" style={{ padding: '48px 40px' }}>
          ...
          <div className="login-mobile-logo" style={{ display: 'none', alignItems: 'center', gap: 10, marginBottom: 28 }}>
            <div style={{ width: 40, height: 40, background: '#1E88E5', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>🛡</div>
            <div>
              <div style={{ fontWeight: 800, fontSize: 16, color: '#0F2044' }}>CMS Portal</div>
              <div style={{ fontSize: 11, color: '#64748B' }}>Complaint Management System</div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            <Field label="Email Address">
              <input style={inputStyle} type="email" value={email} onChange={e => { setEmail(e.target.value); setErr('') }} placeholder="you@company.com" onKeyDown={e => e.key === 'Enter' && handle()} />
            </Field>
          </div>
          <div style={{ marginBottom: 20 }}>
            <Field label="Password">
              <input style={inputStyle} type="password" value={pass} onChange={e => { setPass(e.target.value); setErr('') }} placeholder="••••••••" onKeyDown={e => e.key === 'Enter' && handle()} />
            </Field>
          </div>
          ...
          <button style={{ ...btn('primary'), width: '100%', padding: 14, fontSize: 14 }} onClick={handle} disabled={loading}>
            {loading ? '⏳ Signing in…' : 'Sign In →'}
          </button>

          <div style={{ marginTop: 24, background: '#F8FAFC', borderRadius: 10, padding: '16px 18px', border: '1px solid #E2E8F0' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#64748B', marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.5 }}>Demo Accounts — Tap to fill</div>
            {demos.map(([e, p, role, color]) => (
              <div key={e} onClick={() => { setEmail(e); setPass(p); setErr('') }}
                style={{ padding: '9px 0', cursor: 'pointer', fontSize: 12, borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
