...
function ComplaintCard({ c, users, onOpenComplaint }) {
  const fe = users.find(u => u.id === c.assignedTo)
  return (
    <div
      onClick={() => onOpenComplaint(c.id)}
      style={{ padding: '14px 16px', borderBottom: '1px solid #F1F5F9', cursor: 'pointer', background: '#fff', WebkitTapHighlightColor: 'transparent' }}
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <div>
          <div style={{ fontFamily: 'monospace', fontSize: 11, fontWeight: 700, color: '#1565C0' }}>{c.id}</div>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#0F2044', marginTop: 2 }}>{c.customer}</div>
          <div style={{ fontSize: 12, color: '#64748B', marginTop: 2 }}>{c.city} · {c.type}</div>
        </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {isMobile ? (
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', boxShadow: '0 1px 4px rgba(15,32,68,0.06)', overflow: 'hidden' }}>
          ...
          {recent.map(c => <ComplaintCard key={c.id} c={c} users={users} onOpenComplaint={onOpenComplaint} />)}
        </div>
      ) : (
        <TableWrap>
